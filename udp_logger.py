...
TRANSLATIONS = {
    "de": {
        ...
        "settings_err": "Ungültige Eingabe. Ports: 1–65535, Timeout: 1–3600 s, Auto-Löschen: ≥ 0, Max. Speicher: ≥ 0 GB.",
        ...
        "update_status_ok": "Aktuell", "update_status_available": "Update verfügbar",
        "update_btn_check": "Auf Update prüfen", "update_btn_pull": "Git Pull",
    ...
    "en": {
        ...
        "settings_err": "Invalid input. Ports: 1–65535, Timeout: 1–3600 s, Auto-delete: ≥ 0, Max. storage: ≥ 0 GB.",
        ...
        "update_title": "Software Update",
        "update_status_ok": "Up to date", "update_status_available": "Update available",
        "update_btn_check": "Check for update", "update_btn_pull": "Git Pull",
...
def _storage_cleanup():
    limit_gb = load_settings().get("max_storage_gb", 0)
    if limit_gb <= 0 or not LOG_BASE_DIR.exists():
        return
    ...
    while True:
        ...
        try:
            ...
            add_audit("Speicherlimit: gelöscht",
                      f"{oldest.name} ({fmt_bytes(total)} > {limit_gb} GB)",
                      user="System")
            print(f"[STORAGE] Ordner gelöscht: {oldest.name} (Limit: {limit_gb} GB)")
        except Exception as e:
...
def einstellungen():
    ...
    <div class="card" style="margin-top:16px">
      <div class="card-header"><h2>{t('update_title')}</h2><span id="upd-badge"></span></div>
      <div class="card-body">
        <p id="upd-msg" style="font-size:13px;color:var(--muted);margin-bottom:16px;min-height:18px"></p>
        <div class="actions">
          <button class="btn btn-secondary" id="btn-check" onclick="doCheck()">{t('update_btn_check')}</button>
          <button class="btn btn-primary"   id="btn-pull"  onclick="doPull()"  style="display:none">{t('update_btn_pull')}</button>
        </div>
      </div>
    </div>
    <div class="card" style="margin-top:16px">
    ...
    function setBadge(html) {{ document.getElementById('upd-badge').innerHTML = html; }}

    function doCheck() {{
      var btn = document.getElementById('btn-check');
      btn.disabled = true; btn.textContent = '{t("update_checking")}';
      setMsg(''); setBadge('');
      document.getElementById('btn-pull').style.display = 'none';
      fetch('{url_for("api_check_update")}', {{method:'POST'}})
        .then(function(r){{ return r.json(); }})
        .then(function(d){{
          btn.disabled = false; btn.textContent = '{t("update_btn_check")}';
          if (!d.ok) {{
            setBadge('<span class="badge" style="background:#e65100;color:#fff">Error</span>');
            setMsg(d.msg, 'var(--danger)');
          }} else if (d.up_to_date) {{
            setBadge('<span class="badge badge-green">&#10003; {t("update_status_ok")}</span>');
            setMsg('Local: ' + d.current + '  ·  Remote: ' + d.remote);
          }} else {{
            setBadge('<span class="badge" style="background:#e65100;color:#fff">&#9650; {t("update_status_available")}</span>');
            setMsg('Local: ' + d.current + '  →  Remote: ' + d.remote, '#e65100');
            document.getElementById('btn-pull').style.display = 'inline-block';
          }}
        }})
        .catch(function(e){{ btn.disabled=false; btn.textContent='{t("update_btn_check")}'; setMsg(String(e),'var(--danger)'); }});
    }}

    function doPull() {{
      var btn = document.getElementById('btn-pull');
      btn.disabled = true; btn.textContent = '{t("update_pulling")}';
      setMsg('');
      fetch('{url_for("api_git_pull")}', {{method:'POST'}})
        .then(function(r){{ return r.json(); }})
        .then(function(d){{
          btn.disabled = false; btn.textContent = '{t("update_btn_pull")}';
          if (d.ok) {{
            setMsg('{t("update_pull_ok")} ' + d.msg, 'var(--green)');
            document.getElementById('btn-pull').style.display = 'none';
          }} else {{
            setMsg('{t("update_pull_fail")}: ' + d.msg, 'var(--danger)');
          }}
        }})
        .catch(function(e){{ btn.disabled=false; btn.textContent='{t("update_btn_pull")}'; setMsg(String(e),'var(--danger)'); }});
    }}
    </script>"""
...
def api_check_update():
    ...
    if rc != 0:
        return jsonify({"ok": False, "msg": err or "fetch failed"})
    _, local,  _ = _git("rev-parse", "HEAD")
    _, remote, _ = _git("rev-parse", "origin/main")
    up_to_date = (local == remote) or not local or not remote
    add_audit(t("audit_update_check"),
              f"local={local[:7]}  remote={remote[:7]}  {'up-to-date' if up_to_date else 'UPDATE AVAILABLE'}",
              user=session.get("user", "?"))
    return jsonify({"ok": True, "up_to_date": up_to_date,
                    "current": local[:7], "remote": remote[:7]})
